...
def save(counter=[1]):
    ...
    width = 150  # Reduced from 150 for smaller figure
    height = 150  # Reduced from 150 for smaller figure
    ps = (
        turtle.getcanvas()
        .postscript(
            colormode="color", x=-width // 2, y=-height // 2, width=width, height=height
        )
        .encode("utf-8")
    )
    img = Image.open(io.BytesIO(ps))
    img.load(scale=2)
