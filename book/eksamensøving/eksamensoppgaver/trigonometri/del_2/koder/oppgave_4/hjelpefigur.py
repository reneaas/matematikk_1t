...
def main(dirname, save, rotation_deg=0):
    ...
    x0, y0 = make_angle_arc(B0, radius=0.8, start_angle=start, end_angle=end)
    pts2 = [rotate_point((x0[i], y0[i]), rotation_deg) for i in range(len(x0))]
    x2, y2 = zip(*pts2)
    ax.plot(x2, y2, color="black")
    tx, ty = rotate_point((B0[0] - 1, B0[1] + 1), rotation_deg)
    ...
    ax.text(tx, ty, r"$9$", fontsize=20, ha="center", va="center")

    # Label corners

    ax.text(A[0], A[1], r"$A$", fontsize=20, ha="center", va="top")
    ax.text(B[0], B[1], r"$B$", fontsize=20, ha="left", va="center")
    ax.text(C[0], C[1], r"$C$", fontsize=20, ha="center", va="bottom")
    ax.text(D[0], D[1], r"$D$", fontsize=20, ha="right", va="center")

    ax.plot(*zip(A, C), color="black", linestyle="--")
