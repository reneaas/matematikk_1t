...
def main(dirname, save, rotation_deg=0):
    ...
    BC = 10
    C0 = (
        B0[0] + BC * np.cos(np.deg2rad(angle)),
        B0[1] + BC * np.sin(np.deg2rad(angle)),
    )
    ...
    DC = np.sqrt((C0[0] - D0[0]) ** 2 + (C0[1] - D0[1]) ** 2)
    print(DC)
    ...
    # ax.text(tx, ty, r"$60^\circ$", fontsize=20, ha="center", va="center")
    ...
    # x2, y2 = zip(*pts2)
    # ax.plot(x2, y2, color="black")
    # tx, ty = rotate_point((B0[0] - 0.1, B0[1] + 0.6), rotation_deg)
    # ax.text(tx, ty, r"$150^\circ$", fontsize=20, ha="center", va="center")
    ...
    # start = 30 + 180
    # end = 30 + 180 - 30
    # x0, y0 = make_angle_arc(C0, radius=0.8, start_angle=start, end_angle=end)
    # pts2 = [rotate_point((x0[i], y0[i]), rotation_deg) for i in range(len(x0))]
    # x2, y2 = zip(*pts2)
    # ax.plot(x2, y2, color="black")
    # tx, ty = rotate_point((C0[0] - 1.2, C0[1] - 0.3), rotation_deg)
    ...
    ax.text(tx, ty, r"$8.0$", fontsize=20, ha="center", va="center")

    # BC = 8
    # mid_bc = ((B0[0] + C0[0]) / 2 + 0.25, (B0[1] + C0[1]) / 2 - 0.5)
    # tx, ty = rotate_point(mid_bc, rotation_deg)
    # ax.text(tx, ty, r"$10$", fontsize=20, ha="center", va="center")

    # AD = 4
    # mid_ad = ((A0[0] + D0[0]) / 2 - 0.25, (A0[1] + D0[1]) / 2 + 0.3)
    # tx, ty = rotate_point(mid_ad, rotation_deg)
    # ax.text(tx, ty, r"$4$", fontsize=20, ha="center", va="center")

    # CD
    mid_cd = ((C0[0] + D0[0]) / 2, (C0[1] + D0[1]) / 2 + 0.5)
    tx, ty = rotate_point(mid_cd, rotation_deg)
    ax.text(tx, ty, r"$12.0$", fontsize=20, ha="center", va="center")
...
if __name__ == "__main__":
    ...
    main(dirname=dirname, save=True, rotation_deg=10)
