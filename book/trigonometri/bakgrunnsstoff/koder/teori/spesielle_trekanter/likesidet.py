...
def main(dirname, save):
    ...
    plt.text(
        x=A[0] + 0.5 * 0.5 * (np.cos(angle_A) + 1),
        y=A[1] + 0.45 * 0.5 * np.sin(angle_A),
        s="$60^\\circ$",
        fontsize=fontsize,
        ha="center",
        va="center",
    ...
    plt.text(
        x=B[0] - 0.45 * 0.5 * (np.cos(angle_A) + 1),
        y=B[1] + 0.45 * 0.5 * np.sin(angle_A),
        s="$60^\\circ$",
        fontsize=fontsize,
        ha="center",
        va="center",
    ...
    x, y = make_circle_arc(
        center=C,
        radius=0.3,
        start_angle=np.pi + np.pi / 3,
        end_angle=np.pi + np.pi / 3 + angle_C,
    )
