...
def main(dirname):
    ...
    fignames = ["ingen_løsning.svg", "en_løsning.svg", "to_løsninger.svg"]
    ...
    fig = Figure(
        svg_width * 3,  # Total width for two figures side by side
        svg_height,  # Only one row height needed
        SVG(dirname + "/ingen_løsning.svg").scale(1.25).move(0, 0),
        SVG(dirname + "/en_løsning.svg").scale(1.25).move(svg_width, 0),
        SVG(dirname + "/to_løsninger.svg").scale(1.25).move(2 * svg_width, 0),
    )
