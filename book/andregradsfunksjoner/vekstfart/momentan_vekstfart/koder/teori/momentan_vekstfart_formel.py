...
def main(dirname, save):
    ...
    secant = make_secant_fn(f=f, a=a - dx, b=a + dx)
    # List of functions and their labels.
    functions = [f, T, secant]

    fig, ax = plotmath.plot(
        functions=functions,
        fn_labels=["$f$", "Tangent", "Sekant"],
        xmin=-4,
        xmax=5,
        ymin=-2,
        ymax=6,
    ...
    ax.plot(a + dx, f(a + dx), "ko", markersize=8, alpha=0.7)
    ax.text(
        s=r"$(a + \Delta x, f(a + \Delta x))$",
        x=a + dx - 0.1,
        y=f(a + dx) + 0.05,
        ha="right",
        va="center",
